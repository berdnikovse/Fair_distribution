...
vector <int> apply_mask(const vector <vector <int>>& possible, const vector <size_t>& mask)
{
	vector <int> answer;
...
vector <vector<int>> get_options(const map<string, vector<string>>&solved, map <string, int>&stud_id)
{
...
vector<int> count_score(const vector<int> &students, unsigned size)
{
...
map<string, string> translate_result(const vector<int> &distribution, vector<string> &problems, vector<string> &students)
{
...
vector<int> map_to_vector(const map<first_T, int> &table)
{
	vector <int> answer;
	for (auto it = table.begin(); it != table.end(); it++)
	{
...
map <string, int> get_id(const map <string, sec_T>& table)
{
...
double count_disp(const vector <int>& score)
{
	double avg = 0, disp = 0;
...
bool limit_exceeded(const vector <int>& score, unsigned prob_limit)
{
...
int tot_solved(const vector<int> &students)
{
...
bool advance_mask(vector <size_t>& mask, const vector <vector <int>>& masked_object)
{
